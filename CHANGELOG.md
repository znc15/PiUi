...
- refactor: 优化代码格式，提升可读性 (326e077)
- chore: bump version to 0.1.0-beta.5 (b920cbb)
- fix(ui): 本地 bridge 启动窗口内抑制「无法连接服务器」横幅，消除启动闪烁 (64a5cbd)
- fix(desktop): Windows GUI 下 home_dir 回退 USERPROFILE，修复认证误判与配置浮层闪现 (e7013fd)
- chore: bump version to 0.1.0-beta.4 (f17e250)
- fix: 检测 models.json 自定义 provider，修复已配置却弹「未配置」引导浮层 (32f6da2)
- chore: bump version to 0.1.0-beta.3 (36ca4fc)
- fix(dev): vite 忽略 src-tauri/target，避免 dev 启动与 cargo 并发构建抢锁 EBUSY (1d799bb)
- fix(desktop): 去掉 bridge 入口脚本路径的 \?\ 前缀，修复 Windows 安装版启动崩溃 (92ea122)
- chore: bump version to 0.1.0-beta.2 (7e71c7a)
...
- chore: bump version to 0.1.0-beta.1 (757f067)
