...
export const ChatArea = memo(
  forwardRef<ChatAreaHandle, ChatAreaProps>(
    (
      ...
      const showImmediatePending =
        isStreaming &&
        lastVisibleMessage?.info.role === 'user' &&
        !timeline.some(item => item.kind === 'process-shell' && item.isActive)
      ...
      return (
        <div className="h-full overflow-hidden contain-strict relative">
          ...
          >
            ...
            {showImmediatePending && (
              <div className={`w-full ${maxWidthClass} mx-auto ${paddingClass} pt-3`} role="status" aria-live="polite">
                <div className="assistant-stream-placeholder message-assistant-enter">
                  <span className="assistant-stream-placeholder-label">{t('chatArea.waitingForResponse')}</span>
