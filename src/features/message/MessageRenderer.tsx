...
const UserMessageView = memo(function UserMessageView({
  ...
  return (
    <div
      ...
      className={`message-user-enter flex flex-col items-end group ${hasUserHtmlArtifact ? 'w-full' : ''}`}
    >
...
const AssistantMessageView = memo(function AssistantMessageView({
  ...
  const endedReasoningIds = useMemo(() => {
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      for (let j = i + 1; j < parts.length; j++) {
        ...
        // Pi bridge pre-creates an empty text part together with reasoning.
        // Only real text/tool output means reasoning has ended.
        const nextPart = parts[j]
        if (nextPart.type === 'text' && !nextPart.text.trim()) continue
        if (nextPart.type === 'step-start') continue
  ...
  // The assistant shell arrives before the first provider delta. Keep a visible
  // live state so an empty stream never looks like a stalled send.
  const emptyStreamingPlaceholder =
    isStreaming &&
    renderItems.length === 0 &&
    (processContentScope === 'all' || processContentScope === 'process' || processContentScope === 'inline')

  // process/final 拆完后可能为空；活跃 process 保留回复占位。
  if (
    renderItems.length === 0 &&
    processContentScope !== 'all' &&
    processContentScope !== 'inline' &&
  ...
  return (
    <div ref={wrapperRef} className={`message-assistant-enter flex flex-col ${MSG_SPACING.stack} w-full group`}>
      {emptyStreamingPlaceholder && (
        <div className="assistant-stream-placeholder" role="status" aria-live="polite">
          <span className="assistant-stream-placeholder-label">{t('processing')}</span>
          <span className="assistant-stream-dots" aria-hidden="true"><i /><i /><i /></span>
        </div>
      )}
...
const ToolGroup = memo(function ToolGroup({
  ...
  return (
    <div ref={stepsRootRef} className={`tool-group-body flex flex-col${!isStreaming && !hasActiveTools ? ' is-settled' : ''}`}>
      {showStepsHeader &&
        (descriptiveToolSteps ? (
          <button
            type="button"
